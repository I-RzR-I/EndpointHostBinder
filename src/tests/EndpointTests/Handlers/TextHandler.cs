...
//  ***********************************************************************
//  <copyright file="TextHandler.cs" company="RzR SOFT & TECH">
...
{
    internal sealed class TextHandler : IEndpointHostRequestHandler
    {
        /// <summary>The exact text body emitted by this handler.</summary>
        public const string Body = "hello text";

        /// <inheritdoc />
        public Task<IEndpointHostResult> RequestProcessAsync(
            HttpContext context, CancellationToken cancellationToken = default)
            => Task.FromResult(EndpointResults.Text(Body));

        /// <inheritdoc />
        public IEndpointHostResult RequestProcess(HttpContext context)
            => EndpointResults.Text(Body);
    }
}